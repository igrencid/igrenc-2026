...
<section class="mx-auto grid max-w-6xl gap-8 px-5 py-10 lg:grid-cols-2">
    <div>
        ...
        <form action="{{ route('checkout.store') }}" method="POST" class="glass mt-8 space-y-4 rounded-2xl p-6">
            @csrf
            ...
            <div class="rounded-2xl border border-purple-500/20 bg-purple-500/10 p-4 text-sm text-slate-300">
                <div class="font-bold text-purple-300">Alur Pembayaran</div>
    ...
    <div class="glass rounded-2xl p-6">
        <h2 class="text-2xl font-black">Ringkasan</h2>
            ...
            @foreach($cart as $row)
                <div class="flex justify-between border-b border-white/10 pb-3">
                    <div>
                        ...
                        <div class="text-sm text-slate-400">Qty {{ $row['quantity'] }}</div>
                    </div>
        ...
        <div class="mt-6 rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-4 text-sm text-yellow-200">
            Pesanan belum otomatis berhasil setelah checkout. Status akan berubah setelah user upload bukti dan admin memverifikasi pembayaran.
        </div>
    </div>
</section>
@endsection