    ...
    <div class="glass rounded-[2rem] p-8">
        ...
        <div class="flex flex-col justify-between gap-5 md:flex-row md:items-center">
            <div>
                ...
                <p class="mt-2 text-slate-400">Invoice: {{ $order->invoice_number }}</p>
            </div>
        ...
        <div class="mt-8 grid gap-5 md:grid-cols-2">
            <div class="rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <div class="mt-4 space-y-2 text-slate-300">
                    ...
                    <div>Whatsapp: {{ $order->customer_phone ?? $order->customer_whatsapp ?? '-' }}</div>
                </div>
            </div>
        ...
        ">
            ...
            @if(! in_array($paymentStatus, ['paid', 'waiting_verification']))
                <form action="{{ route('orders.upload-proof', $order->invoice_number) }}" method="POST" enctype="multipart/form-data" class="mt-5 space-y-4">
                    @csrf
